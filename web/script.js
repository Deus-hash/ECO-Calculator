    ...
    function generateReport(fmt) {
      ...
      const html = '<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>ECO.Calculator — Отчёт о расчёте</title>' +
        ...
        rows.map(r => '<tr><td>' + (r.n||'') + '</td><td>' + r.name + '</td><td>' + r.rho + '</td><td>' + (document.body.dataset.mode === 'engineer' ? (parseFloat(r.delta)*1000).toFixed(6) : (parseFloat(r.delta)*1000).toFixed(0)) + '</td><td>' + r.M + '</td><td>' + r.C + '</td><td>' + r.E + '</td></tr>').join('') +
